...
def plot_2d_histogram(df, mach_min, mach_max, cl_min, cl_max,
    ...
    # Save plots
    save_dir = "Plots"
    filename = f"{save_dir}/B77x_Mach_CL_hist_weights_{n_points}pts.png"
...
def main():
    # Load data
    df = pd.read_csv("data/B77x_histogram_data.csv")
    mach_bin_size, cl_bin_size = get_bin_sizes(df)

    # Domain bounds
    mach_min, mach_max = 0.78, 0.86
    cl_min, cl_max = 0.51, 0.57
    filter_radius = 4
